...
- 生产 demo 使用 SQLite 持久化会话和运行快照，可运行多个 Uvicorn worker；数据库文件位于 `outputs/spatial-agent.db`，横向扩展时应迁移到独立共享数据库。
...
- 计划步骤使用 `{"$from":"步骤ID","path":"结果字段"}` 引用前一步结构化结果。
- 引用必须声明对应 `depends_on`，并且来源步骤必须先执行；非法引用在计划解析阶段拒绝。
- 已支持“查询洪山区行政区边界并分析 DEM 高程概况”三步链路，前一步行政区结果会绑定到区域栅格统计。
- Console 步骤卡片展示前置依赖和结构化结果摘要，可直接观察数据流向。

## M33 当前能力

- 多步骤任务采用 fail-fast 策略：工具失败后停止后续调用。
- 已完成步骤保留真实结果，失败步骤标记为 FAILED，未执行步骤标记为 BLOCKED 并记录原因。

## M34 当前能力

- 支持对失败 Run 从第一个失败步骤恢复，不重新调用 Planner，也不重复成功步骤。
- API 提供 `POST /runs/{run_id}/retry`，Console 失败结果提供“重试失败步骤”按钮。

## M35 当前能力

- API 和 artifact 增加安全的 `provenance` 摘要，记录步骤依赖、输入绑定、执行策略和结果引用。
- Console 增加“运行血缘”面板，展示复合分析的数据流，不暴露原始参数、几何或凭据。

## M36 当前能力

- 评测报告增加总步骤耗时、Planner 耗时、Token 总量和依赖链有效率。
