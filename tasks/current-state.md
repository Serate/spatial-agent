...
- 状态：M337-A～D 已实现并推送；本地/Docker 紧凑契约 10/10，通过 strict、compileall 和 readiness 验证
- 基线：`dea1180`
...
- M337-A/B 已完成：架构守卫输出不可变三类清单、分类 schema 和逐模块分类，并对 shim 执行 AST 形状及 80 行上限检查。
- M337-C/D 已完成：阶段文档、兼容矩阵、问题日志、热状态和文档索引已同步；本地/Docker 验证均通过。
- 下一步按项目全局目标评估兼容 facade、状态投影和历史回归债务，确定 M338 入口。
- M336 已收尾；不删除历史 import、不移动生产模块，不扩大为默认全量回归。
