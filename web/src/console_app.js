    ...
    function renderRun(data) {
      ...
      setStatus(data.status);
      const envelope=data.result||{};
      $('title').textContent=envelope.title||data.plan?.goal||statusName(data.status);
      $('subtitle').textContent=envelope.summary||data.resolved_request||data.request;
      $('decisionMode').textContent=decisionMode(data);
      $('answer').textContent=envelope.summary||data.answer||'暂无最终答案。';
      $('answer').className='answer'+(envelope.summary||data.answer?'':' muted');
    ...
    function renderActionWorkbench() { if(!window.ConsoleActionHost){ $('actionWorkbenchBody').innerHTML='<div class="distribution-note">Action Host 不可用。</div>'; return; } window.ConsoleActionHost.mount({target:$('actionWorkbenchBody'),catalog:actionCatalog,initialValues:{planner:$('planner').value,backend:$('backend').value},invoke:executeDomainAction,onResult:async(data,action)=>{ if(responseDomain(data)!==currentDomainId()) return; setStatus(data.status||'COMPLETED'); appendMessage('assistant',data.result?.summary||data.answer||action.label+' 已完成'); renderActionExecution(data); $('actionWorkbench').open=false; loadHistory(responseDomain(data)); }}); }
    ...
    async function retryRun(runId) { const domainId=domainForRun(runId); setStatus('EXECUTING'); $('subtitle').textContent='正在从失败步骤恢复，已完成步骤不会重复执行。'; try { const body=withDomainPayload({planner:$('planner').value,backend:$('backend').value,export_artifact:true,export_geojson:true},domainId); const response=await nativeFetch(domainPath('/runs/'+encodeURIComponent(runId)+'/retry',domainId),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); const data=await response.json(); if(!response.ok) throw new Error(responseError(data,'重试失败')); rememberRunDomain(data,domainId); if(domainId===currentDomainId()){ renderRun(data); appendMessage('assistant',data.answer||data.error||statusName(data.status)); } } catch(e) { if(domainId===currentDomainId()){ setStatus('FAILED'); $('error').innerHTML='<div class="error">'+escapeHtml(e.message)+'</div>'; appendMessage('system',e.message); } } }
    async function resolveRunDecision(decisionId, choice, version) { if(!decisionId) return; const domainId=decisionDomains.get(String(decisionId))||responseDomain(lastRunData); document.querySelectorAll('.decision-action-bar button').forEach(button=>button.disabled=true); try { const body=withDomainPayload({choice,expected_version:version,planner:$('planner').value,backend:$('backend').value},domainId); const response=await nativeFetch(domainPath('/decisions/'+encodeURIComponent(decisionId)+'/resolve',domainId),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); const data=await response.json().catch(()=>({})); if(!response.ok) throw new Error(responseError(data,'决策提交失败')); rememberRunDomain(data,domainId); if(domainId===currentDomainId()){ renderRun(data); appendMessage('assistant',data.answer||data.error||statusName(data.status),data.run_id); } } catch(error) { if(domainId===currentDomainId()){ $('error').innerHTML='<div class="error">'+escapeHtml(error.message)+'</div>'; document.querySelectorAll('.decision-action-bar button').forEach(button=>button.disabled=false); } } }
    ...
    function renderCanonicalInteraction(data){
      ...
      window.ConsoleActionHost.mount({
        ...
        onResult:async result=>{ renderRun(result); appendMessage('assistant',result.answer||result.error||statusName(result.status),result.run_id); await hydrateAutoDomainState(result); },
      });
      return true;
    }
    ...
    async function sendChat(text) {
      ...
      try {
        ...
        if(mode===selectedDomainModeId()&&generation===domainGeneration&&viewGeneration===conversationGeneration){ renderRun(data); appendMessage('assistant',data.answer||data.error||statusName(data.status),data.run_id); if(unboundAuto&&data.domain_id) await hydrateAutoDomainState(data); }
      } catch(e) {
