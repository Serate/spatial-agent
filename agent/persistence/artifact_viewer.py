...
def _evidence_section(artifact: Dict[str, Any]) -> str:
    ...
    result_completeness = projection.get("completeness") or {}
    ...
    result_state = {
        ...
        "pending": "进行中",
    }.get(str(result_completeness.get("state") or "unknown"), "未知")
    recovery_actions = "、".join(
        str(item)[:64]
        for item in (recovery.get("allowed_actions") or [])
    ) or "无"
    return """<section><h2>证据索引（Evidence Registry）</h2>
<p class="muted">结果：{result_state} · Registry：{registry_state} · {count} 个入口 · 证据索引：{complete_state} · 迁移状态：{migration_state}</p>
...
<ul>{entries}</ul></section>""".format(
        ...
        result_state=html.escape(result_state),
