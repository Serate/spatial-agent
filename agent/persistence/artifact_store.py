...
from agent.application.service_async import normalize_async_result_evidence
...
class ArtifactStore:
    ...
    def __init__(self, root: str | None = None, *, legacy_domain_id: str = "gis"):
        configured_root = (
            root
            if root is not None
            else os.environ.get("SPATIAL_AGENT_ARTIFACT_ROOT", "outputs/runs")
        )
    ...
    def write_run(self, payload: Dict) -> str:
        ...
        artifact = {
            ...
            "completeness": (
                nested_result.get("completeness")
                if isinstance(nested_result, dict)
                else None
            ),
        ...
        _atomic_write_text(path, json.dumps(artifact, ensure_ascii=True, indent=2))
        return path.as_posix()
