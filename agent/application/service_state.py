...
class ServiceState:
    ...
    def __init__(
        ...
        self._tool_approval_store = (
            SQLiteToolApprovalStore(state_db_path)
            if state_db_path
            else InMemoryToolApprovalStore()
        )
    ...
    @property
    def tool_approval_store(self):
        return self._tool_approval_store
    ...
    def runtime(self, planner: str, backend: str) -> Any:
        ...
        with self._runtime_lock:
            ...
            if accepts_kwargs or "approval_store" in parameters:
                kwargs["approval_store"] = self._tool_approval_store
