...
<!-- archived-block:document-index-restructure -->
来源：tasks/task-progress.md
归档时间（UTC）：2026-08-28T07:06:46.1751291Z

### 文档索引架构重构 — 已完成

- 目标：建立热状态、阶段包、稳定知识、历史归档四层索引；恢复脚本按阶段只读取必要内容。
- 已完成：归档旧总快照、旧总计划和旧任务账本；创建 `docs/document-index.json`、`docs/README.md`、`docs/stages/` 和 `tasks/current-state.md`；压缩 `agent-work-state.md` 与 `tasks/plan.md`。
- 验证：`validate_document_index.ps1` 通过；`resume_context.ps1` 默认恢复和 M323 主题查询通过；默认上下文仅包含 4 个入口文件。
- 交付：旧总快照、总计划和任务账本已保存在 `docs/archive/`，新热状态与 M323 阶段包已建立。
<!-- archived-block-end:document-index-restructure -->
