...
- `agent/runtime.py` 的内存状态和能力目录职责已迁入 `agent/runtime_state.py` 与 `agent/runtime_core/capabilities.py`；规划、校验、执行、重规划和恢复仍待继续物理拆分。
...
3. Runtime 规划/执行/生命周期/投影拆分；projection、planning、execution、control、state、capability seams 已建立，计划修复/执行重规划/恢复仍待收敛。
