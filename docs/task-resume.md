...
- Current work: M80.2 长期记忆已完成——新增 agent/memory.py（fact memory，内存/SQLite 双模式）+ SQLite memory_facts 表 + ContextBuilder 注入既往结论 + GET /memory 双入口 + 前端记忆卡片；11 项专项 + 全量 510 项通过；容器内真实链路验证（两次运行沉淀 2 条记忆）。下一步按 M80.3（标准可观测性）规划。
