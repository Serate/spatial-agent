...
- Current work: M80.2 长期记忆进行中——跨会话可检索的结论记忆（fact memory）：新增 agent/memory.py + SQLite memory_facts 表 + ContextBuilder 注入既往结论 + GET /memory 契约 + 前端记忆卡片；规划已写入 docs/milestones.md。
