...
- Current work: M81.1 成本治理与并发配额已完成——新增 agent/cost_governance.py（TokenBudget：会话 token 预算/单次运行 cap/并发配额，默认关闭零变化）+ service 接入 + 429/budget/concurrency_limited 错误契约 + /metrics 摘要；11 项专项 + 全量 537 项通过。下一步按 M81 规划（A3 工具动态扩展 / C9 流式输出 / D13 收尾）。
