...
进入 M59.2：跨进程运行与结果证据验收。M59.1 已完成统一能力目录、跨入口 `/capabilities` 契约和能力驱动全局评测；M58.3 的 Docker/Compose 业务验收也已完成。

1. 将能力目录中的环境要求接入评测 optional gating，区分 planner/tool 成功和真实 GIS 能力完成。
2. 增加生产 SQLite 会话、运行、重试、取消和结果引用的跨进程契约矩阵。
3. 细化真实 artifact 几何、边界几何、无几何演示和截断不可绘制等证据状态，并接入 Console 与全局报告。
