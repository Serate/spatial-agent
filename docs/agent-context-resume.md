...
- M315 已将右侧聊天区接入实时答案气泡：等待期间显示循环点状动效，首个答案事件到达后
  复用同一气泡逐字输出，终态不重复追加，并支持减少动态效果设置。
- M316 已确认并修复旧恢复路径复用 2048 token 上限的问题：紧凑恢复改用一次有界的
  4096～8192 token 预算和 `temperature=0`；同一会话真实 DEM 请求已 `COMPLETED`，
  `compact_recovery_attempts=1`，答案已形成。
- M317 已将用户答案预算独立提高到默认 4096 token，普通答案可见上限提高到 6000 字符；
  Docker 定向回归通过，未重复调用真实模型。
- M316 已完成阶段收口并按全局目标重新规划下一阶段；若 Provider 配置再次失败，记录安全错误 receipt，
  不重复重试。
- 详细恢复入口仍是 `docs/agent-work-state.md` 与 `tasks/task-progress.md`；当前开发方式为
  单 Agent、最大并发度 1。恢复时只读取 M314 Spec/Plan、当前快照、任务账本尾部和明确文件。
- M317 代码提交 `8f9bf93` 已提交；若继续工作，先检查 `git status`、任务账本当前/最近区块和
  当前 Goal 状态，不批量读取历史阶段文档或全量源码。
