...
## 2026-08-30 M331 上下文预算裁剪丢失版本化 workflow 摘要

### 现象

- `ContextBuilder(max_chars=12000)` 接收完整 workflow template 摘要时，直接把超预算的整个目录替换为 `omitted`；调用方因此读取不到预期的 `schema_version`，上下文回归失败。

### 根因与处理

- 裁剪策略只按 section 粗粒度删除，没有为版本化能力目录提供结构化压缩层。
- 现在超预算时先保留目录版本、模板身份、工具/结果边界和步骤形状，再继续按预算删除低优先级 section；执行校验仍使用 Domain-owned 完整目录，不由压缩摘要授权。

### 预防

- 版本化 Planner context 在整体省略前必须有结构化 compact projection；回归同时断言预算上限、schema version 和关键能力边界，不能只断言“不泄露”。

## 2026-08-30 M331 前端 ReAct 事件与答案流上限漂移

### 现象

- Python Runtime 已产生 ReAct 事件，但 Console 事件消费者未接受对应 kind，工具提案等待审批等进展不可见；答案流前端仍以 1800 字符为上限，而后端答案契约允许更长文本。

### 根因与处理

- RunEvent 的 Python/JavaScript allowlist 没有同步更新；前端逐字缓冲上限和后端 `answer_length` 仍沿用早期体验限制。
- 现在补齐 ReAct event kind 的共享可见性，并把答案显示/事件长度统一到 6000 字符；结构化结果和工具参数仍须先完整校验，事件只携带安全摘要。

### 预防

- 新增事件 kind 必须同时更新 Python contract、Console normalizer 和 smoke；答案上限应集中定义或至少在跨层测试中锁定，不能仅修改模型输出上限。

## 2026-08-30 M331 真实复杂规划的 provider 延迟

### 现象

- 真实模型短直答可以流式完成，但复杂 GIS 多步请求在 45 秒有界验收中没有返回；原配置的 Planner timeout 为 180 秒，人工中止后未创建可报告的阶段终态。

### 判断与处理

- 当前证据只支持 provider 规划延迟/复杂结构化输出耗时，不能断言是 GIS 算法或 Runtime 逻辑故障。已保留通用直答和离线多步契约作为可重复基线，未保存模型原文或密钥。
...
## 2026-08-30 Docker 网络不可达导致实时新闻搜索降级

### 现象

- 用户询问“最近比较火的 AI 新闻”时，Agent 识别出需要联网，但 `web_search` 连续重试后返回
  `unavailable`，最终说明无法获取实时新闻。

### 根因与判断

- 搜索配置已启用，Provider 为 `https://html.duckduckgo.com/html/`，但 Docker 容器内对该地址的连接在
  8 秒超时；模型 Provider 可用不代表搜索网络也可用，二者是独立链路。
- 当前返回属于正确的安全降级：没有可验证来源时不能伪造“最新热点”，也不能把模型自身知识当作实时新闻。

### 处理与预防

- 需要联网时，为 Docker 配置可用的宿主机出口或 HTTP/HTTPS 代理，或替换为可访问且经过白名单登记的搜索适配器；
  不能只修改模型 API 地址。
- 修改后应从容器内验证搜索 Provider 的连通性，并执行一次带来源证据的实时搜索验收；若保持离线，应提供带时间戳的本地新闻数据，不能宣称“最近”。

## 2026-08-31 公共网页正文需要与持久化结果隔离

### 现象

- 为支持“搜索来源 → 读取正文 → 生成回答”，`web_fetch` 需要把正文交给当前运行的模型；如果直接把工具返回值写入 `StepRun`、SQLite、Artifact 或事件，网页正文可能进入持久化和公开接口。
- 网页上下文经过普通裁剪后，在极端自定义结果下仍可能超过答案模型上下文上限。

### 根因与处理

- 工具结果既承担安全证据，又承担模型输入时，内部正文句柄容易沿着通用 Result 边界泄漏；多级裁剪如果没有最小合法包，只有“通常足够小”而不是硬上限。
- `WebAccessPolicy` 统一 `off/allowlist/public`、HTTPS、DNS 地址和重定向门禁；`web_fetch` 通过 `_model_context` 暂存正文，Runtime 在投影前移除该字段，只保存标题、URL、域名、哈希、长度、预览和原因码。答案上下文超限时逐级删除正文、事实、步骤，最后退化为固定最小字段包。
- 重启恢复只根据已保存的安全 URL 通过同一 ToolRegistry/WebPolicy 重新抓取；失败时保留结构化降级，不伪造正文可用。
