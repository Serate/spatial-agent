...
## M217-B：历史恢复档案体量过大，软性提示仍可能被忽略

### 现象

恢复时如果同时打开恢复档案、任务档案和问题日志，会一次加载约 825 KB 历史内容；当前阶段信息被淹没，
也会增加上下文压缩和后续误读的概率。

### 处理与预防

以 `docs/agent-context-current.md` 作为唯一启动快照。历史文档保留审计价值，但默认不读取；只有明确阶段或关键词
时才执行 `rg` 定位并读取局部窗口。`scripts/resume_context.ps1` 只输出当前卡、Git 状态和最近提交，便于重复执行最小恢复。
当前卡保持短小，只记录当前切片、阻塞、证据引用和下一步，不复制历史日志、模型响应或完整数据产物。

## M217-C：Docker 测试容器可能滞后于当前工作树

### 现象

宿主测试已修改后，Docker 中仍执行旧断言，导致阶段回归出现与当前源码不一致的失败。

### 根因

生产 compose 使用镜像内的 `/app`，不是自动绑定宿主工作树；仅重启旧容器不会包含新提交或未提交修改。
...
从 M217 起进一步采用“最小恢复模式”：当前卡控制在约 4 KB 内，历史全文默认读取数为 0，最多读取一个按关键词定位的历史片段；源码和测试也必须先定位再局部读取。若当前任务需要扩大读取预算，必须先说明需要的证据和原因，不能以“了解背景”为理由批量加载文件。
...
## M217-A：生命周期证据不能被误当成跨入口核心结果

### 现象

请求与计划 identity 相同的两条流程，在比较完整结果时出现 interaction receipt、decision id 和 transition evidence 差异。

### 根因

一条流程通过 `provide_facts` 继续旧请求，另一条流程直接提交计划确认；它们共享请求/计划语义，但生命周期动作本来不同。旧测试把整份生命周期证据当成可字节相等的核心结果。

### 处理与预防

跨入口 harness 只比较公共语义核心；interaction receipt、用户确认和 transition evidence 继续作为各自生命周期的可读证据单独验证。不要为了让比较通过而删除这些证据，也不要把不同动作伪装成同一状态。
...
生产 Console 的复杂地图 smoke 中，点击“清空对话”后偶尔仍显示旧地图、行政区选区和工作区结果。重试可能通过，导致问题看起来像浏览器或地图渲染不稳定。

### 原因

`clearChat()` 先 `await /sessions/{session_id}/clear`，完成后才调用 `resetConversationView()`。当会话清理请求延迟时，前端状态清理被推迟，用户看到的仍是上一轮结果；这不是 GIS 几何或 Leaflet 图层本身的问题。

### 修复与约束

先同步清理地图实例、空间上下文、结果面板和对话视图，再异步持久化清理；持久化失败只显示有界错误提示，不恢复旧结果。浏览器回归必须同时检查点击后立即状态和等待异步请求后的状态，避免只检查最终页面而漏掉时序回归。

## M207：Preview 只返回 status，缺少统一生命周期投影

### 现象

未匹配能力的 `/runs` 响应可以通过 `result.lifecycle` 表达“需要澄清”和允许动作，但 `/runs/preview` 只有 `status` 和计划证据。前端或其他调用方如果只消费 preview，必须自行推断生命周期，容易与正式执行路径产生漂移。
...
## 上下文压缩后重复读取历史档案导致读取量过大

### 现象

新对话或上下文压缩后，如果同时全文读取恢复文档、任务档案、问题日志和里程碑，模型会消耗大量上下文预算在历史细节上，反而降低当前阶段代码定位和验证效率；宽泛的全文搜索也会把大量旧阶段命中一次性带入上下文。

### 根因

长期审计记录与当前工作状态没有被严格区分，且历史追溯缺少输出上限。把“了解项目背景”误认为“读取所有历史”会造成重复信息、旧结论和当前状态混杂。
