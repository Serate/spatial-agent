...
- 每个大阶段先按模块边界拆成可独立验收的子任务；无依赖的子任务可以并行，当前最大并发数为 4。
...
- 历史记录：M60 当时最多拆成 5 个并行子任务；当前全局规则已调整为最大并发度 4。
...
M67 先拆分为最多 4 个边界清晰的子任务，公共协议由主线统一集成；阶段完成后执行离线、GIS、真实模型可选 live、Docker 和浏览器联合验收并创建版本。
