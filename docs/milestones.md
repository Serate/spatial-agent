...
## M285：开放式 Planner 多工具编排纵向切片（已完成）
...
- Docker M285/M283 精简联合回归 **13/13**，compileall、architecture strict、生产 readiness 通过；四次单请求真实中转 Composite probe 均安全拒绝且未创建 run，错误码为 `plan_response_field_invalid`、`plan_components_unexpected`、`taskplan_policy_unavailable`、`capability_not_registered`，已记录到中文问题日志。
- 修复了 Composite Planner 成功/非成功组件状态提示、Planner context 的受限 `tools` 投影，以及 live probe 的有界 `--max-output-tokens` 参数；未放宽 unknown fields、能力 allowlist 或非成功 outcome 的组件约束。
- 阶段结论：TaskPlan bridge、跨入口 evidence 和 fail-closed execution gate 已完成；真实中转模型仍需下一阶段做 provider/context/Planner 格式适配，不能把 provider 可达或 HTTP 200 当作多步计划成功。
- M285 按“完整能力切片、集中精简验收”规则收口；下一阶段从模型适配、能力选择稳定性、开放式成功率和跨入口体验七维度全局重规划。

## M286：中转模型 Planner 适配与能力身份稳定性（已完成）

- 全局规划不新增专题工具或数据，集中处理 provider 可达与合法多步计划之间的适配缺口；能力图、Spec、Plan 已建立。
- 当前先为 context 和公共 capability projector 增加可复制的 `domain_id`、`capability_id`、`selection_key` 以及受限工具/结果摘要，并在 LLM 提示中要求精确复制身份，不得从 label 或工具名猜测。
- Planner evidence 增加有界的跨域选择键；Composite live 的 `--max-output-tokens` 现在能沿懒加载 Production Composition Root 生效；超过 8 个组件的计划拒绝而不静默截断。
...
## M189：Action Effect 跨入口一致性验收（已完成）

M189 将 M188 的 Action Effect 从结构化投影验证推进到真实 Service 持久化与恢复链路验证，覆盖 HTTP、history、Artifact、async、SQLite 重启和多 worker CAS。

- 新增 `tests/test_m189_action_effect_cross_entry.py`，只保留 3 个高价值场景：Service/Artifact/history/async/restart equality、HTTP/detail/Artifact equality、双 worker 重复动作 replay equality。
- M189 专项 **3/3**；M184–M188 受影响联合回归 **28/28**；Docker compileall、quick、stage、full-stage 和生产 acceptance 均通过，容器 healthy，核心/可选数据 ready。
- 验证结果表明同一 Action Effect 在不同入口只保留稳定动作语义；run ID、replay 标记和传输方式变化不会产生误报；重复提交不会重复 dispatch。
- 本阶段没有新增 GIS 工具、前端专用分支或 live token；继续保持默认测试离线、精简，真实模型/GIS/Docker 仅作为显式验收路径。

## M190 全局重规划参考

下一阶段从“跨入口证据一致”转向“开放式请求能力发现与真实模型闭环”：

1. 产品：支持未预定义表达的能力匹配、结构化澄清、计划预览、有限修复和用户确认。
2. 架构：强化 RequestFacts、Capability Catalog、Workflow 和 Planner 之间的动态 discovery seam，减少固定问句规则。
3. 数据：让 Domain 只提供能力、数据健康和证据适配器，公共 Runtime 不携带 GIS 专用意图分支。
4. 模型：用脱敏 replay 和最小 live-short 验证真实模型输出 schema、工具选择、DAG 依赖、澄清和修复 lineage。
...
## M196-A：Capability-level Evidence Provider projection（已完成，待版本推送）

- 新增领域无关 `spatial-agent.capability-catalog-evidence.v1` 投影；`project_capability_catalog_evidence()` 将静态 Capability Catalog 与 Evidence Provider 的 per-capability observation 合并，统一输出 readiness、coverage、alignment、provenance、availability 和 bounded missing reasons。
- Runtime 的 `runtime_capabilities()`、能力上下文和后续前端消费现在使用同一 `capability-evidence.v1`；只保留有界摘要，不把 Provider 原始 payload、路径或来源值复制到公共 evidence。缺失、过期、冲突、未知 schema 和非数据 Domain 的 `not_applicable` 均有明确降级语义。
- Text、GIS 和自定义 Provider 契约测试共 **5/5**；M168、M140、M194、M195 受影响回归合计 **31/31**；Docker quick、ci、stage、full-stage、compileall、production acceptance、Docker Node smoke、宿主 Node 语法和 Chrome/CDP workflow evidence smoke 均通过。
- 新发现的 `not_applicable` 误降级问题已记录到 `docs/agent-development-issues.md`。本切片没有增加 GIS Runtime 分支、默认 CI 网络访问、私有配置或真实模型 token。

## M196-B：全局规划参考

1. **产品**：把 capability-level evidence 接入开放式请求的候选卡片、澄清和结果工作区，让用户看到“可用/降级/需补事实/不可用”及下一步动作。
2. **架构**：统一 runtime snapshot、per-run workflow selection 和 Artifact/SQLite replay 的 capability evidence projection，避免同一 Provider 被重复探测或不同入口各自归一化。
3. **数据**：让 Evidence Provider 显式表达更新时间、覆盖、来源绑定、跨源冲突和有效期；验证缺失、过期、未对齐和冲突的可恢复降级。
4. **模型**：把脱敏 capability evidence 摘要纳入开放式 Planner replay；模型不得选择不可用能力，信息不足时生成结构化 clarification/selection，而不是固定问句分支。
...
1. **产品**：让至少两个 Domain 通过各自目录完成组合能力发现、计划预览、执行、结果组合和 Artifact 恢复；未知能力继续返回结构化澄清。
2. **架构**：把 GIS catalog/allowlist 从 `agent/` 物理下沉到 Domain Pack；HTTP workflow validate/revise 和 Runtime 所有校验都显式接收 Domain catalog，公共层只保留声明式校验算法。
3. **数据**：为组合组件统一记录覆盖、时效、来源、冲突和复用/重验状态，缺失或过期时驱动通用阻断/澄清/repair。
4. **模型**：用脱敏 replay 验证多 Domain 目录选择、组合 DAG、坏计划和有限 repair；显式 live-short 只验证模型消费 bounded context，不保存原始响应。
5. **部署**：验证组合请求在 HTTP、Async、SQLite restart、Artifact-only 接管中保持幂等、结果/evidence 一致，并覆盖 Domain/provider 不可用。
...
1. **产品**：让两个 Domain 的组合能力都能从发现、预览、确认、执行到结果/Artifact 恢复闭环，而不是只有 HTTP catalog 可替换。
2. **架构**：统一组合组件的 plan identity、DAG、ToolRegistry governance、Result/Evidence 和 Action Receipt，避免只在 GIS Domain 维护组合状态。
3. **数据**：把组件级覆盖、时效、来源、冲突和重验状态接入通用 Evidence Provider，驱动澄清、阻断或有限 repair。
4. **模型**：用脱敏 replay 验证多 Domain 目录选择、坏组合计划、工具失败与 repair lineage；再做一次显式 live-short。
...
- 新增版本化 `spatial-agent.component-fact-handoff.v1`；Planner 选择的组件从公共 catalog requirements、RequestFacts 和 workflow constraints 生成有界 known facts、missing fields、effective constraints 和下一步动作，Domain preview 只消费公共 handoff。
- 新增签名、过期和字段白名单约束的单组件 continuation；补充事实后重新构建 context、重新 Planner、重新执行 completeness/TaskPlan/DAG gate，篡改 token、未知字段、请求/领域/组件不匹配均 fail closed，未完成澄清不创建 run。
- HTTP `composite_plan`、同步/异步提交、planning evidence、artifact/async safe projection、Composite View 和 Console 均保留脱敏 continuation identity；前端显示组件缺失字段和补充提示，不显示 token、prompt 或模型原文。
