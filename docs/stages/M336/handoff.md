...
- 阶段：`M336` HTTP 入口收敛
- 状态：M336-A～D 已实现并完成 Docker 定向验收，待提交推送
...
2. 后续全局重规划时评估兼容 facade、状态投影和历史测试债务。
...
- 新增 `agent/application/fastapi_http.py`，集中 FastAPI 的依赖解析、共享分发、错误投影、SSE 和 artifact 响应；生产入口保留公开路由与历史函数名，但不再复制这些传输实现。
- Domain Routing catalog/select/override/clear 已改为通过共享 route metadata 与 `HTTPApplication` 分发，FastAPI 与 stdlib 使用同一语义动作。
...
- 入口收敛：`production_api.py`、`agent/application/fastapi_http.py`、`agent/application/http_routes.py` 及 M336 结构契约测试。
